...
export function DeleteTaskConfirmationDialog({
  ...
  const handleDelete = async () => {
    ...
    try {
      const response = await fetch(
        `/api/v1/properties/${propertyId}/housekeeping/${task.id}`,
        { method: "DELETE" },
      )
      ...
      const message =
        deleteError instanceof Error ? deleteError.message : "Failed to delete housekeeping task."
      toast({
  ...
  return (
    <AlertDialog open={open} onOpenChange={onOpenChange}>
      <AlertDialogContent>
        <AlertDialogHeader>
          <AlertDialogTitle>Delete housekeeping task?</AlertDialogTitle>
          <AlertDialogDescription>
