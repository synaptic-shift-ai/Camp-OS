...
import { AlertCircle, Loader2, CheckCircle, DollarSign, Calendar, Users, Home } from 'lucide-react'
...
export function CheckInDialog({ open, onOpenChange, reservation }: CheckInDialogProps) {
  ...
  const handleCheckIn = async () => {
    ...
    try {
      // Validate payment method if balance is due
      if (hasBalance && !paymentMethodId.trim()) {
        setError('Payment method is required when balance is due')
      ...
      const response = await fetch(`/api/admin/reservations/${reservation.id}/check-in`, {
        ...
        body: JSON.stringify({
          balance_amount: hasBalance ? outstandingBalance : undefined,
          payment_method_id: hasBalance ? paymentMethodId.trim() : undefined,
          check_in_notes: checkInNotes.trim() || undefined,
        }),
      })
            ...
            <div className="space-y-3">
              ...
              <div className="space-y-2">
                <Label htmlFor="payment-method">
                  Payment Method ID (Stripe) <span className="text-destructive">*</span>
                </Label>
                <Input
                  id="payment-method"
                ...
                <p className="text-xs text-muted-foreground">
                  Enter the Stripe PaymentMethod ID to charge ${balanceInDollars}
                </p>
              </div>
            </div>
          )}
