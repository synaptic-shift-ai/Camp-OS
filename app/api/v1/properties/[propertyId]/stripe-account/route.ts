...
import { StripeConnectInfo } from '@/src/modules/PropertyManagement/domain/StripeConnectInfo'
...
export async function DELETE(
  ...
  try {
    ...
    if (updateError) {
      ...
      return NextResponse.json(
        error(ERROR_CODES.INTERNAL_ERROR, 'Failed to disconnect Stripe'),
        { status: 500 }
      )
    }
    ...
    return NextResponse.json(
      error(ERROR_CODES.INTERNAL_ERROR, 'Failed to disconnect Stripe account', {
        message: err.message,
      }),
      { status: 500 }
    )
