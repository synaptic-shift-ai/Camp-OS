...
export async function POST(
  ...
  try {
    ...
    if (propertyError || !property) {
      return NextResponse.json(
        error(ERROR_CODES.AUTH_003, 'Forbidden - property not found or access denied'),
        { status: 403 }
      )
    }
    ...
    return NextResponse.json(
      error(ERROR_CODES.INTERNAL_ERROR, 'Failed to create reservation', {
        message: err.message,
      }),
      { status: 500 }
    )
...
export async function GET(
  ...
  try {
    ...
    return NextResponse.json(
      error(ERROR_CODES.INTERNAL_ERROR, 'Failed to fetch reservations', {
        message: err.message,
      }),
      { status: 500 }
    )
