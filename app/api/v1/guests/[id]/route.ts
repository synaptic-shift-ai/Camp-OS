...
export async function GET(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const guestDTO = await queryHandler.execute({ guestId: params.id })
    ...
    return NextResponse.json(
      error(ERROR_CODES.INTERNAL_ERROR, 'Failed to fetch guest', {
        message: err.message,
      }),
      { status: 500 }
    )
...
export async function PATCH(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    return NextResponse.json(
      error(ERROR_CODES.INTERNAL_ERROR, 'Failed to update guest', {
        message: err.message,
      }),
      { status: 500 }
    )
