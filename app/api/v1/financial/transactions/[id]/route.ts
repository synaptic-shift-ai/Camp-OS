...
export async function GET(
  ...
  try {
    ...
    if (!transaction) {
      return NextResponse.json(
        error(ERROR_CODES.RESOURCE_NOT_FOUND, 'Transaction not found'),
        { status: 404 }
      )
    }
    ...
    if (propertyError || !property || property.company_id !== company.id) {
      return NextResponse.json(
        error(ERROR_CODES.AUTH_003, 'Forbidden - transaction belongs to different company'),
        { status: 403 }
      )
    }
    ...
    return NextResponse.json(
      error(ERROR_CODES.INTERNAL_ERROR, 'Failed to get transaction', {
        message: err.message,
      }),
      { status: 500 }
    )
