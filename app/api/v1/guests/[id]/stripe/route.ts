...
export async function POST(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    if (authError || !user) {
      return NextResponse.json(error(ERROR_CODES.AUTH_001, 'Unauthorized'), { status: 401 })
    }

    // Verify guest exists
    const repository = new SupabaseGuestRepository(supabase)
    const queryHandler = new GetGuestQueryHandler(repository)
    const existingGuestDTO = await queryHandler.execute({ guestId: params.id })
    ...
    if (companyError || !company) {
      return NextResponse.json(
        error(ERROR_CODES.AUTH_003, 'Forbidden - guest belongs to different company'),
        { status: 403 }
      )
    }
    ...
    // Fetch updated guest
    const updatedGuestDTO = await queryHandler.execute({ guestId: params.id })
    ...
    if (err.message.includes('not found')) {
      return NextResponse.json(
        error(ERROR_CODES.RESOURCE_NOT_FOUND, 'Guest not found'),
        { status: 404 }
      )
    }
    ...
    if (err.message.includes('already linked')) {
      return NextResponse.json(
        error(ERROR_CODES.VALIDATION_ERROR, 'Stripe customer already linked to this guest'),
        { status: 409 }
      )
    }

    return NextResponse.json(
      error(ERROR_CODES.INTERNAL_ERROR, 'Failed to link Stripe customer', {
        message: err.message,
      }),
      { status: 500 }
    )
