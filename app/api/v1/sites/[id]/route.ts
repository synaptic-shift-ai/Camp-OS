...
import { GetSiteQuery } from '@/modules/SiteManagement/application/queries/GetSiteQuery'
import { UpdateSiteCommand } from '@/modules/SiteManagement/application/commands/UpdateSiteCommand'
...
export async function GET(
  ...
  try {
    ...
    if (!site) {
      return NextResponse.json(
        error(ERROR_CODES.RESOURCE_001, 'Site not found'),
        { status: 404 }
      )
    }
    ...
    if (!company) {
      return NextResponse.json(
        error(ERROR_CODES.AUTH_002, 'Access denied to this site'),
        { status: 403 }
      )
    }
...
export async function DELETE(
  ...
  try {
    ...
    if (err.message.includes('has active reservations')) {
      return NextResponse.json(
        error(ERROR_CODES.VALIDATION_003, err.message),
        { status: 400 }
      )
    }
