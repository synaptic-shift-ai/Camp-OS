-- ============================================================
-- Make Supabase Cron secret optional for staging/dev
-- ============================================================
-- When CRON_SECRET is not configured in the app environment, cron routes
-- allow POST requests without requiring a matching bearer token. This migration
-- lets setup_cron_jobs be called with only the app URL for that case:
--   SELECT setup_cron_jobs('https://staging-url.com');
--
-- When CRON_SECRET is configured, rerun setup with the matching secret:
--   SELECT setup_cron_jobs('https://staging-url.com', 'real-secret-value');
-- ============================================================
...
BEGIN
  ...
  IF v_app_url IS NULL THEN
    RAISE EXCEPTION 'Cron app_url not set. Run setup_cron_jobs() first.';
  END IF;

  v_headers := jsonb_build_object('Content-Type', 'application/json');

  IF NULLIF(v_cron_secret, '') IS NOT NULL THEN
    v_headers := v_headers || jsonb_build_object('Authorization', 'Bearer ' || v_cron_secret);
  END IF;
