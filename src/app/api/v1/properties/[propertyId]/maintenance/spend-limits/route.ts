...
import { CreateSpendLimitRequestSchema } from '@/types/api/v1/schemas/maintenance'
...
export async function GET(
  ...
  try {
    ...
    return success({ spendLimits }, request)
  } catch (err) {
    const message = err instanceof Error ? err.message : 'Unknown error'
    console.error('[Spend Limits API v1] GET error:', err)
...
export async function POST(
  ...
  try {
    ...
    const parsed = CreateSpendLimitRequestSchema.safeParse(body)
    ...
    const queries = new MaintenanceQueries(supabase as unknown as SupabaseClient)
    const spendLimit = await queries.createSpendLimit({
      propertyId,
    ...
    const message = err instanceof Error ? err.message : 'Unknown error'
    console.error('[Spend Limits API v1] POST error:', err)
    return error(ErrorCodes.INTERNAL_ERROR, request, { message })
  }
}
