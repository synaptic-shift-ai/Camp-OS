...
export async function GET(
  ...
  try {
    ...
    if (!invoice) {
      return NextResponse.json(
        error(ERROR_CODES.RESOURCE_NOT_FOUND, 'Invoice not found'),
        { status: 404 }
      )
    }
    ...
    if (propertyError || !property || property.company_id !== company.id) {
      return NextResponse.json(
        error(ERROR_CODES.AUTH_003, 'Forbidden - invoice belongs to different company'),
        { status: 403 }
      )
    }
    ...
    return NextResponse.json(
      error(ERROR_CODES.INTERNAL_ERROR, 'Failed to get invoice', {
        message: err.message,
      }),
      { status: 500 }
    )
