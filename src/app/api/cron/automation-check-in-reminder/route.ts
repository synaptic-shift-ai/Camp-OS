/**
 * Automation Check-In Reminder Cron
 *
 * POST /api/cron/automation-check-in-reminder
 *
 * Daily cron (8am) that fires `system.check_in_reminder` automations
 * for reservations checking in today.
 *
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(`[Check-In Reminder] Running for date=${today}`)

    // Find confirmed reservations checking in today
    const { data: reservations, error: resError } = await supabase
      .from('reservations')
      .select('id, property_id, guest_id, site_id, confirmation_number, check_in_date, status')
      .eq('check_in_date', today)
      .eq('status', 'confirmed')

    if (resError) {
      console.error('[Check-In Reminder] Failed to fetch reservations:', resError)
    ...
    if (!reservations || reservations.length === 0) {
      console.log('[Check-In Reminder] No confirmed reservations checking in today')
      return NextResponse.json({
        success: true,
    ...
    console.log(`[Check-In Reminder] Found ${reservations.length} reservations checking in today`)
    ...
    for (const reservation of reservations) {
      ...
      if (!propertyId) {
        console.warn(`[Check-In Reminder] Skipping reservation ${reservation.id} — no property_id`)
        continue
      }
      ...
      if (!companyId) {
        console.warn(`[Check-In Reminder] Skipping reservation ${reservation.id} — no company_id on property`)
        continue
      }
      ...
      const { data: existingLogs } = await supabase
        ...
        .eq('event_type', 'system.check_in_reminder')
      ...
      try {
        ...
        if (result.matched > 0) {
          console.log(`[Check-In Reminder] Reservation ${reservation.id}: matched=${result.matched} passed=${result.passed} executed=${result.executed}`)
        }
      } catch (err) {
        console.error(`[Check-In Reminder] Pipeline failed for reservation ${reservation.id}:`, err)
        // Continue processing other reservations
      }
    }

    console.log(`[Check-In Reminder] Complete: ${reservations.length} reservations, ${skipped} deduplicated, ${totalExecuted} actions executed`)
    ...
    console.error('[Check-In Reminder] Unexpected error:', error)
    return NextResponse.json(
      {
