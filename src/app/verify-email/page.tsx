...
function VerifyEmailContent() {
  ...
  const [redirectTarget, setRedirectTarget] = useState<"login" | "account">("login")
  ...
  useEffect(() => {
    const init = async () => {
      if (isConfirmed) {
        ...
        const target = u ? (redirect || "/company-details") : "/login"
        setRedirectTarget(target === "/login" ? "login" : "account")
        setConfirmed(true)
    ...
    init()
  }, [supabase, router, isConfirmed, linkError, redirect, searchParams])
  ...
  const handleResend = async () => {
    if (!email) {
      setError("Email is required to resend. Go back to sign up.")
      return
    }
    setResending(true)
    setResent(false)
