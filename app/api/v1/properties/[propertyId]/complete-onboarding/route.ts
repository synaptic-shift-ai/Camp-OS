...
import { GetPropertyQueryHandler } from '@/src/modules/PropertyManagement/application/queries/GetPropertyQuery'
import { UpdatePropertyCommandHandler } from '@/src/modules/PropertyManagement/application/commands/UpdatePropertyCommand'
import { SupabasePropertyRepository } from '@/src/modules/PropertyManagement/infrastructure/SupabasePropertyRepository'
import { toPropertyDTO } from '@/src/modules/PropertyManagement/application/DTOs/PropertyDTO'
...
export async function POST(
  ...
  try {
    ...
    if (property.companyId !== company.id) {
      return NextResponse.json(
        error(ERROR_CODES.AUTH_003, 'Forbidden - property belongs to different company'),
        { status: 403 }
      )
    }
    ...
    if (err.message.includes('not found')) {
      return NextResponse.json(
        error(ERROR_CODES.RESOURCE_NOT_FOUND, err.message),
        { status: 404 }
      )
    }
    ...
    if (err.message.includes('already complete')) {
      return NextResponse.json(
        error(ERROR_CODES.VALIDATION_ERROR, 'Onboarding already completed'),
        { status: 400 }
      )
    }

    return NextResponse.json(
      error(ERROR_CODES.INTERNAL_ERROR, 'Failed to complete onboarding', {
        message: err.message,
      }),
      { status: 500 }
    )
