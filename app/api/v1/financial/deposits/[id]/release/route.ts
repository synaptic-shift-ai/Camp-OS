...
export async function POST(
  ...
  try {
    ...
    if (!existingDeposit) {
      return NextResponse.json(
        error(ERROR_CODES.RESOURCE_NOT_FOUND, 'Security deposit not found'),
        { status: 404 }
      )
    }
    ...
    if (propertyError || !property || property.company_id !== company.id) {
      return NextResponse.json(
        error(ERROR_CODES.AUTH_003, 'Forbidden - deposit belongs to different company'),
        { status: 403 }
      )
    }
    ...
    return NextResponse.json(
      error(ERROR_CODES.INTERNAL_ERROR, 'Failed to release security deposit', {
        message: err.message,
      }),
      { status: 500 }
    )
